...
public class Stage4 {
    ...
    public void readConfiguration(Scanner in) {
        ...
        for (int i = 0; i < numDoors; i++) {
            ...
            doors.add(d);
            if (i == 0)
                central.addNewSensorZ0(d.getSensor());
            else
                central.addNewSensorZ1(d.getSensor());
        }
        int numWindows = in.nextInt();
        for (int i = 0; i < numWindows; i++) {
            Window w = new Window();
            windows.add(w);
            central.addNewSensorZ1(w.getSensor());
        }
        int numPirs = in.nextInt();

        // Creamos PIR y agregamos a la lista
        for (int i = 0; i < numPirs; i++) {
            ...
            pirs.add(p);
            central.addNewSensorZ2(p.getSensor());
        ...
        while (true) {
            ...
            switch (command.charAt(0)) {
                ...
                case 'p': //Movemos personas
                    ...
                    //Verificamos si persona al moverse se encuentra en el radio de algun sensor
                    for (int idx = 0; idx < pirs.size(); idx++)
                        pirs.get(idx).detectar(persons.get(i));

            }
            //Checkeamos si se viola la alarma
            central.checkZone();
        }
    }
